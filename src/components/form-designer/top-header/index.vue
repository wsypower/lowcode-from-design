  ...
  <el-header class="top-header">
    <div class="left">
      ...
      <el-tooltip effect="light" content="撤销" placement="bottom">
        <svg-icon icon-class="op-undo" :class-name="`undo ${undoDisabled ? 'disabled' : ''}`"
          @click="undo" /></el-tooltip>

      <el-tooltip effect="light" content="重做" placement="bottom">
        <svg-icon icon-class="op-redo" :class-name="`redo ${redoDisabled ? 'disabled' : ''}`" @click="redo" />
      </el-tooltip>
    </div>
    <div class="middle">
      <el-tooltip effect="light" content="PC端" placement="bottom">
        <svg-icon icon-class="op-pc" :class-name="designer.isPCLayout ? 'active' : ''"
          @click="designer.setPCLayout()" /></el-tooltip>

      <el-tooltip effect="light" content="移动端" placement="bottom">
        <svg-icon icon-class="op-mobile" :class-name="designer.isH5Layout ? 'active' : ''"
          @click="designer.setH5Layout()" />
      </el-tooltip>

      <template v-if="designer.isPCLayout">
        <el-select :model-value="designer.isFormWidthCustomize ? '自定义' : designer.pcFormWidth
          " @change="onPCSizeChange" can-edit>
          <el-option v-for="(size, index) in designer.screenWidthList" :key="size"
            :label="index !== 0 ? `${size}px` : size" :value="size" />
        </el-select>
        <el-input-number v-model="designer.pcFormWidth" v-if="designer.isFormWidthCustomize" controls-position="right" />
      </template>
      <template v-else>
        <el-select :model-value="designer.h5CurrentPhone.name" @change="onPhoneChange" can-edit>
          <el-option v-for="(phone, index) in designer.phoneSizeList" :key="phone.name" :label="phone.name"
            :value="index" />
        </el-select>
      </template>
    </div>
    <div class="right">
      <v-form-render v-if="designerMounted" :form-json="formJson" ref="renderRef"
        style="position: absolute; top: -200%; left：200%;">
      </v-form-render>

      <el-tooltip effect="light" content="清空表单组件" placement="bottom">
        <svg-icon icon-class="op-delete" class-name="delete-icon" @click="clearFormWidget" /></el-tooltip>
      <i class="divider"></i>

      <el-tooltip effect="light" content="预览表单" placement="bottom">
        <svg-icon icon-class="op-preview" @click="showPreviewDialog" /></el-tooltip>
      <el-tooltip effect="light" content="导入JSON" placement="bottom">
        <svg-icon icon-class="op-import" @click="showImportDialog" /></el-tooltip>
      <el-tooltip effect="light" content="导出JSON" placement="bottom">
        <svg-icon icon-class="op-export" @click="showExportDialog" /></el-tooltip>
      <el-tooltip effect="light" content="导出代码" placement="bottom">
        <svg-icon icon-class="op-code" @click="showCodeDialog" /></el-tooltip>
      ...
      <el-tooltip effect="light" content="保存" placement="bottom">
        ...
        <svg-icon v-else icon-class="op-save" :class-name="`upload-icon ${!hasWidgets || isPublished ? 'disabled' : ''
          }`" @click="onSave" /></el-tooltip>

      <el-tooltip v-if="!isTemplateMode" effect="light" content="发布" placement="bottom">
        <el-icon v-if="isPublishing" class="loading-icon is-loading">
          <Loading />
        </el-icon>
        <svg-icon v-else icon-class="op-upload" :class-name="`upload-icon ${!hasWidgets || isPublished ? 'disabled' : ''
          }`" @click="publishForm" /></el-tooltip>
    ...
    <preview-dialog :visible="previewDialogVisible" :designer="designer" :globalDsv="globalDsv"
      @close="hidePreviewDialog" />

    <import-dialog :visible="importDialogVisible" :designer="designer" :globalDsv="globalDsv" @close="hideImportDialog" />

    <export-dialog :visible="exportDialogVisible" :designer="designer" :globalDsv="globalDsv" @close="hideExportDialog" />

    <code-dialog :visible="codeDialogVisible" :designer="designer" :globalDsv="globalDsv" @close="hideCodeDialog" />
  </el-header>
</template>
...
const isTemplateMode = computed(() => !!templateId.value);
...
async function onSave() {
  isTemplateMode.value ? persistTempalte() : persistForm();
}
...
async function persistForm(showTip = true) {
